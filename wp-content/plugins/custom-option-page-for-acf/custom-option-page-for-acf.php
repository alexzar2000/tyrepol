...
require_once plugin_dir_path(__FILE__) . 'copacf-functions.php';
...
/**
 * Sync saved ACF values to wp_options using get_field() so each field's return format is respected.
 */
function copacf_sync_fields_to_options($post_id) {
    ...
    foreach ($field_groups as $field_group) {
        ...
        if (!empty($fields) && is_array($fields)) {
            copacf_sync_field_values_to_options($post_id, $fields);
        }
    }
}

function copacf_sync_field_values_to_options($post_id, array $fields) {
    foreach ($fields as $field) {
        ...
        update_option("copacf_options_{$field['name']}", $field_value);

        if (!empty($field['key'])) {
            update_option("copacf__options_{$field['name']}", $field['key']);
        }
    }
}
...
function copacf_admin_init() {
    ...
    if (isset($_POST['acf_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['acf_nonce'])), 'acf_form_nonce')) {
        if (!current_user_can($current_page['capability'])) {
            return;
        }
        ...
        if (acf_validate_save_post(true)) {
            ...
            if ($acf_post_id) {
                ...
                copacf_sync_fields_to_options($acf_post_id);
            }
        }
    }
